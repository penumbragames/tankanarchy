/**
 * Helper functions for runtime type enforcement for enums.
 * @author omgimanerd
 */
...
/**
 * Type enforcement identity function for declaring a object map whose keys must
 * be values in an enum type. Throws an error if the object contains a key that
 * is NOT an enum type.
 *
...
/**
 * Type enforcement identity function for declaring an object map whose keys
 * must contain all the values in an enum. Throws an error if the given object
 * does not contain keys for all the values in enumType, or if the object
